...
import TypeCorrespondantService from '../services/TypeCorrespondantService';
...
const TypeCorrespondant = () => {  
  ...
  const onFormSubmit = (values) => {
    ...
    if(!isAddMode){ //MOD
      TypeCorrespondantService.update(values).then((res)=>{
        if(res.status !== 200){ 
      ...
      TypeCorrespondantService.create(values).then((res)=>{
  ...
  const deleteSelectedItem = () => {
    ...
    let _items = [...items]; //on copie les items;
    let message = "Une erreur est survenue veuillez réessayer ultérieurement";
    TypeCorrespondantService.delete(element).then((res) => {
      if(res.status !== 200){
        toast.current.show({severity:'error', detail: message, life:'3000'});
      }else{
  ...
  const itemsLoading = (params) => {
    setLoading(true);
    TypeCorrespondantService.getAll(params).then((data) => {
      setTotal(data.total);
  ...
  return (
            ...
            <div className="p-col-12">
              <div className="card">
                <div className="entete">
                  <h5>Types correspondant</h5>
                  <Button className="p-button-primary" onClick={() => afficheDialog(null)} icon="pi pi-plus"></Button>
                </div>
      ...
      {/* Les Dialog (popup) */}
      <Dialog header='type courrier' style={{ width: '50vw'}} footer={dialogFooter} onHide={hideDialog} visible={showDialog}> 
          ...
          <div className="p-fluid">
            <div className="p-formgrid p-grid">
              ...
              <div className="p-field p-col-8">
                <label htmlFor='libelle' className="p-d-block">Libelle</label>
...
export default TypeCorrespondant;