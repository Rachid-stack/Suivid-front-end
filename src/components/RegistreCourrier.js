...
import RegistreCourrierService from '../services/RegistreCourrierService';

const RegistreCourrier = () => {  
  ...
  const onFormSubmit = (values) => {
    ...
    if(!isAddMode){ //MOD
      RegistreCourrierService.update(values).then((res)=>{
        if(res.status !== 200){ 
      ...
      RegistreCourrierService.create(values).then((res)=>{
  ...
  const deleteSelectedItem = () => {
    ...
    let _items = [...items]; //on copie les items;
    let message = "Une erreur est survenue veuillez réessayer ultérieurement";
    RegistreCourrierService.delete(element).then((res) => {
      if(res.status !== 200){
        toast.current.show({severity:'error', detail: message, life:'3000'});
      }else{
  ...
  const itemsLoading = (params) => {
    setLoading(true);
    RegistreCourrierService.getAll(params).then((data) => {
      setTotal(data.total);
  ...
  return (
      ...
      {/* Les Dialog (popup) */}
      <Dialog header='registre courrier' style={{ width: '50vw'}} footer={dialogFooter} onHide={hideDialog} visible={showDialog}> 
