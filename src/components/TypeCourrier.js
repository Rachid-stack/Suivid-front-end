...
import TypeCourrierService from '../services/TypeCourrierService';
...
const TypeCourrier = () => {  
  ...
  const onFormSubmit = (values) => {
    ...
    if(!isAddMode){ //MOD
      TypeCourrierService.update(values).then((res)=>{
        if(res.status !== 200){ 
      ...
      TypeCourrierService.create(values).then((res)=>{
  ...
  const deleteSelectedItem = () => {
    ...
    let _items = [...items]; //on copie les items;
    let message = "Une erreur est survenue veuillez réessayer ultérieurement";
    TypeCourrierService.delete(element).then((res) => {
      if(res.status !== 200){
        toast.current.show({severity:'error', detail: message, life:'3000'});
      }else{
  ...
  const itemsLoading = (params) => {
    setLoading(true);
    TypeCourrierService.getAll(params).then((data) => {
      setTotal(data.total);
